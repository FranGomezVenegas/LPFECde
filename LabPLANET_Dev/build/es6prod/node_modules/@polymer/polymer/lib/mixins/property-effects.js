define(["exports","../utils/boot.js","../utils/mixin.js","../utils/path.js","../utils/case-map.js","./property-accessors.js","./template-stamp.js","../utils/settings.js"],function(_exports,_boot,_mixin,_path,caseMap,_propertyAccessors,_templateStamp,_settings){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.PropertyEffects=void 0;caseMap=babelHelpers.interopRequireWildcard(caseMap);/**
@license
...
*/ /* for notify, reflect */ /* for annotated effects */ /** @const {Object} */const CaseMap=caseMap;// Monotonically increasing unique ID used for de-duping effects triggered
 ...
 */function pathMatchesTrigger(path,trigger){if(trigger){let triggerPath=trigger.name;return triggerPath==path||trigger.structured&&(0,_path.isAncestor)(triggerPath,path)||trigger.wildcard&&(0,_path.isDescendant)(triggerPath,path)}else{return!0}}/**
 ...
 * @private
 */function notifyPath(inst,path,props){let rootProperty=(0,_path.root)(path);if(rootProperty!==path){let eventName=(0,caseMap.camelToDashCase)(rootProperty)+"-changed";dispatchNotifyEvent(inst,eventName,props[path],path);return!0}return!1}/**
...
a.structured=(0,_path.isPath)(arg);if(a.structured){a.wildcard=".*"==arg.slice(-2);if(a.wildcard){a.name=arg.slice(0,-2)}}}return a}/**
 ...
 * @private
 */function marshalArgs(data,args,path,props){let values=[];for(let i=0,l=args.length;i<l;i++){let arg=args[i],name=arg.name,v;if(arg.literal){v=arg.value}else{if(arg.structured){v=(0,_path.get)(data,name);// when data is not stored e.g. `splices`
...
if(value!==node[prop]||"object"==typeof value){node[prop]=value}}/**
     ...
     */_setPendingProperty(property,value,shouldNotify){let isPath=this.__dataHasPaths&&(0,_path.isPath)(property),prevProps=isPath?this.__dataTemp:this.__data;if(this._shouldPropertyChange(property,value,prevProps[property])){if(!this.__dataPending){this.__dataPending={};this.__dataOld={}}// Ensure old is captured from the last turn
...
if("property"===kind){name=(0,caseMap.dashToCamelCase)(name)}addBinding(this,templateInfo,nodeInfo,kind,name,parts,literal);return!0}else{return super._parseTemplateNodeAttribute(node,templateInfo,nodeInfo,name,value)}}/**
     * Overrides default `TemplateStamp` implementation to add support for
...
if(lastIndex&&lastIndex<text.length){let literal=text.substring(lastIndex);if(literal){parts.push({literal:literal})}}if(parts.length){return parts}else{return null}}/**
     ...
     * @protected
     */static _evaluateBinding(inst,part,path,props,oldProps,hasPaths){let value;if(part.signature){value=runMethodEffect(inst,path,props,oldProps,part.signature)}else if(path!=part.source){value=(0,_path.get)(inst,part.source)}else{if(hasPaths&&(0,_path.isPath)(path)){value=(0,_path.get)(inst,path)}else{value=inst.__data[path]}}if(part.negate){value=!value}return value}}// make a typing for closure :P
