define(["exports"],function(_exports){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.lastAttributeNameRegex=_exports.createMarker=_exports.isTemplatePartActive=_exports.Template=_exports.rewritesStyleAttribute=_exports.markerRegex=_exports.nodeMarker=_exports.marker=void 0;/**
 * @license
 ...
 * in template.
 */_exports.marker=marker;const nodeMarker=`<!--${marker}-->`;_exports.nodeMarker=nodeMarker;const markerRegex=new RegExp(`${marker}|${nodeMarker}`);_exports.markerRegex=markerRegex;const rewritesStyleAttribute=(()=>{const el=document.createElement("div");el.setAttribute("style","{{bad value}}");return"{{bad value}}"!==el.getAttribute("style")})();/**
 * An updateable Template that tracks the location of dynamic parts.
 */_exports.rewritesStyleAttribute=rewritesStyleAttribute;class Template{constructor(result,element){this.parts=[];this.element=element;let index=-1,partIndex=0;const nodesToRemove=[],_prepareTemplate=template=>{const content=template.content,walker=document.createTreeWalker(content,133/* NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_COMMENT |
...
for(const n of nodesToRemove){n.parentNode.removeChild(n)}}}_exports.Template=Template;const isTemplatePartActive=part=>-1!==part.index;// Allows `document.createComment('')` to be renamed for a
// small manual size-savings.
_exports.isTemplatePartActive=isTemplatePartActive;const createMarker=()=>document.createComment("");/**
 ...
 *    * (") then any non-("), or
 *    * (') then any non-(')
 */_exports.createMarker=createMarker;const lastAttributeNameRegex=/([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F \x09\x0a\x0c\x0d"'>=/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/;//# sourceMappingURL=template.js.map
