...
}selectedIncubator(e){console.log("selectedIncubator");var data=[];data.name="INC_1";_store.store.dispatch((0,_emDemoA_actions.setSelectedIncubator)(data))}batchSelected(e){if(!e.detail.value)return;//console.log('batchSelected', 'e.detail.value', e.detail.value);
this.selectedBatch=e.detail.value;_store.store.dispatch((0,_emDemoA_actions.setSelectedBatch)(e.detail.value));return}onFinalTokenFilled(){this.callBackRefreshWindow=this.refreshWindow.bind(this);console.log("em-demo-a-batches >> calling getActiveBatches within onFinalTokenFilled");this.getActiveBatches({finalToken:this.finalToken,schemaPrefix:this.schemaPrefix});/*        if (this.selectedBatch){
