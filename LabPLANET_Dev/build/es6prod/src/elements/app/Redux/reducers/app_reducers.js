...
*/function navigation(state="",action){switch(action.type){case _app_actions.UPDATE_PAGE:return _objectSpread({},state,{page:action.page});default:return state;}}const InitialUserState={loggedIn:!1,appLanguage:"en",finalToken:"",userDB:"",userID:"",appProcedureList:[]};function user(state=InitialUserState,action){switch(action.type){case _app_actions.DO_LOGIN:return _objectSpread({},state,{loggedIn:!0,finalToken:action.token,userDB:action.userDB,userID:action.userID});case _app_actions.DO_LOGOUT:return{InitialUserState,loggedIn:!1};case _app_actions.SET_APP_LANGUAGE:return _objectSpread({},state,{appLanguage:action.appLanguage});case _app_actions.SET_APP_PROCEDURE_LIST://console.log('SET_APP_PROCEDURE_LIST', action);
