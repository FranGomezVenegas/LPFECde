 ...
 */class PasswordFieldElement extends TextFieldElement{static get is(){return"vaadin-password-field"}static get version(){return"2.1.6"}static get properties(){return{/**
...
const thisTemplate=DomModule.import(this.is+"-template","template"),revealButton=thisTemplate.content.querySelector("[part=\"reveal-button\"]"),styles=thisTemplate.content.querySelector("style"),inputField=memoizedTemplate.content.querySelector("[part=\"input-field\"]");inputField.appendChild(revealButton);memoizedTemplate.content.appendChild(styles)}return memoizedTemplate}ready(){super.ready();this.focusElement.type="password";this.focusElement.autocapitalize="off";this.addEventListener("focusout",()=>{if(!this._passwordVisibilityChanging){this._setPasswordVisible(!1);if(this._cachedChangeEvent){this._onChange(this._cachedChangeEvent)}}})}_onChange(e){if(this._passwordVisibilityChanging){this._cachedChangeEvent=e}else{this._cachedChangeEvent=null;super._onChange(e)}}_revealButtonMouseDown(e){if(this.hasAttribute("focused")){e.preventDefault()}}_togglePasswordVisibilityTouchend(e){// Cancel the following click event
e.preventDefault();this._togglePasswordVisibility();this.focusElement.focus()}_togglePasswordVisibility(){this._passwordVisibilityChanging=!0;this.focusElement.blur();this._setPasswordVisible(!this.passwordVisible);this.focusElement.focus();this._passwordVisibilityChanging=!1}_passwordVisibleChange(passwordVisible){this.focusElement.type=passwordVisible?"text":"password"}}customElements.define(PasswordFieldElement.is,PasswordFieldElement);export{PasswordFieldElement};