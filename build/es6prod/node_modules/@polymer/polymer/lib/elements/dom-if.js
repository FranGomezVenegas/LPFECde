define(["exports","../../polymer-element.js","../utils/templatize.js","../utils/debounce.js","../utils/flush.js","../utils/async.js","../utils/path.js"],function(_exports,_polymerElement,_templatize,_debounce,_flush,_async,_path){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.DomIf=void 0;/**
@license
...
let observer=new MutationObserver(()=>{if(this.querySelector("template")){observer.disconnect();this.__render()}else{throw new Error("dom-if requires a <template> child")}});observer.observe(this,{childList:!0});return!1}this.__ctor=(0,_templatize.templatize)(template,this,{// dom-if templatizer instances require `mutable: true`, as
...
this.__invalidProps=this.__invalidProps||Object.create(null);this.__invalidProps[(0,_path.root)(prop)]=!0}}}})}if(!this.__instance){this.__instance=new this.__ctor;parentNode.insertBefore(this.__instance.root,this)}else{this.__syncHostProperties();let c$=this.__instance.children;if(c$&&c$.length){// Detect case where dom-if was re-attached in new position
