...
this.getProcedureList({finalToken:this.finalToken});_store.store.dispatch((0,_session_actions.addSession)(data));var actionName="getAppHeader",paramsUrl="actionName="+actionName+"&finalToken="+this.finalToken+"&personFieldsName="+this.userInfoId,datas=[];datas.finalToken=this.finalToken;datas.actionName=actionName;datas.paramsUrl=paramsUrl;this.openTabsOnLogin(this.userTabsOnLogin);this.getAppHeader(datas);_store.store.dispatch((0,_app_actions.doLogin)(this.finalToken,this.userName,this.userInfoId,this.userTabsOnLogin))}fieldButtonClicked(e){if("buttonAccess"==e.detail.buttonName){this.login()}}onListChange(e){if("userRole"==e.detail.name){this.doLogin(e.detail.value)}}doLogin(userRole){this.userRole=userRole;this.ajaxFinalToken({actionName:"finaltoken",partialToken:this.partialToken,userRole:userRole})}openTabsOnLogin(tabsInfo){var i;for(i=0;i<tabsInfo.length;i++){//if (tabsInfo[i].tabName!='user-profile'){
...
}}_store.store.dispatch((0,_tabs_actions.setCurrentTab)(curTab))}constructor(){super();return;var curTab=[];this.userName="labplanet";this.userInfoId="1";//return;      
