 ...
 * @private
 */function notifyPath(inst,path,props){let rootProperty=root$0(path);if(rootProperty!==path){let eventName=camelToDashCase$0(rootProperty)+"-changed";dispatchNotifyEvent(inst,eventName,props[path],path);return!0}return!1}/**
 ...
 * @return {void}
 * @private
 */function runNotifyEffect(inst,property,props,oldProps,info,hasPaths){let rootProperty=hasPaths?root$0(property):property,path=rootProperty!=property?property:null,value=path?get$0(inst,path):inst.__data[property];if(path&&value===void 0){value=props[property];// specifically for .splices
...
if(!a.literal){a.rootProperty=root$0(arg);// detect structured path (has dots)
a.structured=isPath$0(arg);if(a.structured){a.wildcard=".*"==arg.slice(-2);if(a.wildcard){a.name=arg.slice(0,-2)}}}return a}/**
 ...
 * @private
 */function marshalArgs(data,args,path,props){let values=[];for(let i=0,l=args.length;i<l;i++){let arg=args[i],name=arg.name,v;if(arg.literal){v=arg.value}else{if(arg.structured){v=get$0(data,name);// when data is not stored e.g. `splices`
...
/**
     ...
     * @protected
     */_setPendingPropertyOrPath(path,value,shouldNotify,isPathNotification){if(isPathNotification||root$0(Array.isArray(path)?path[0]:path)!==path){// Dirty check changes being set to a path against the actual object,
...
if(!isPathNotification){let old=get$0(this,path);path=/** @type {string} */set$0(this,path,value);// Use property-accessor's simpler dirty check
...
if(value!==node[prop]||"object"==typeof value){node[prop]=value}}/**
     ...
     */_setPendingProperty(property,value,shouldNotify){let isPath=this.__dataHasPaths&&isPath$0(property),prevProps=isPath?this.__dataTemp:this.__data;if(this._shouldPropertyChange(property,value,prevProps[property])){if(!this.__dataPending){this.__dataPending={};this.__dataOld={}}// Ensure old is captured from the last turn
...
// ----------------------------
     ...
     * @return {void}
     * @public
    */notifySplices(path,splices){let info={path:""},array=/** @type {Array} */get$0(this,path,info);notifySplices(this,array,info.path,splices)}/**
     ...
     *   is undefined.
     * @public
     */get(path,root){return get$0(root||this,path)}/**
     ...
     * @return {void}
     * @public
    */set(path,value,root){if(root){set$0(root,path,value)}else{if(!this[TYPES.READ_ONLY]||!this[TYPES.READ_ONLY][/** @type {string} */path]){if(this._setPendingPropertyOrPath(path,value,!0)){this._invalidateProperties()}}}}/**
     ...
     * @public
     */push(path,...items){let info={path:""},array=/** @type {Array}*/get$0(this,path,info),len=array.length,ret=array.push(...items);if(items.length){notifySplice(this,array,info.path,len,items.length,[])}return ret}/**
     ...
     * @public
     */pop(path){let info={path:""},array=/** @type {Array} */get$0(this,path,info),hadLength=!!array.length,ret=array.pop();if(hadLength){notifySplice(this,array,info.path,array.length,0,[ret])}return ret}/**
     ...
     * @public
     */splice(path,start,deleteCount,...items){let info={path:""},array=/** @type {Array} */get$0(this,path,info);// Normalize fancy native splice handling of crazy start values
...
if(items.length||ret.length){notifySplice(this,array,info.path,start,items.length,ret)}return ret}/**
     ...
     * @return {*} Item that was removed.
     * @public
     */shift(path){let info={path:""},array=/** @type {Array} */get$0(this,path,info),hadLength=!!array.length,ret=array.shift();if(hadLength){notifySplice(this,array,info.path,0,0,[ret])}return ret}/**
     ...
     * @return {number} New length of the array.
     * @public
     */unshift(path,...items){let info={path:""},array=/** @type {Array} */get$0(this,path,info),ret=array.unshift(...items);if(items.length){notifySplice(this,array,info.path,0,items.length,[])}return ret}/**
...
let info={path:""};value=get$0(this,path,info);propPath=info.path}else if(Array.isArray(path)){// Normalize path if needed
...
if(lastIndex&&lastIndex<text.length){let literal=text.substring(lastIndex);if(literal){parts.push({literal:literal})}}if(parts.length){return parts}else{return null}}/**
     ...
     * @protected
     */static _evaluateBinding(inst,part,path,props,oldProps,hasPaths){let value;if(part.signature){value=runMethodEffect(inst,path,props,oldProps,part.signature)}else if(path!=part.source){value=get$0(inst,part.source)}else{if(hasPaths&&isPath$0(path)){value=get$0(inst,path)}else{value=inst.__data[path]}}if(part.negate){value=!value}return value}}// make a typing for closure :P
