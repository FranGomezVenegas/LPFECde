...
var i;for(i=0;i<this.userRoles.length;i++){if(0==i){this.set("formFields.4.items.0.keyName",this.userRoles[0]);this.set("formFields.4.items.0.keyValue_en",this.userRoles[0]);this.set("formFields.4.items.0.keyValue_es",this.userRoles[0])}else{this.push("formFields.4.items",{keyName:this.userRoles[i],keyValue_es:this.userRoles[i],keyValue_en:this.userRoles[i]})}}}initAppSession(){store.dispatch(doLogin(this.finalToken,this.userName,this.userInfoId,this.tabsOpenOnLogin));var data={sessionId:this.appSessionId,userRole:this.userRole,startDate:this.appSessionStartDate,tabsOpenOnLogin:this.tabsOpenOnLogin};//console.log('app-login >> initAppSession', 'data', data, 'this.userRole', this.userRole);
