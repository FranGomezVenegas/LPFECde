 ...
 * @mixinFunction
 * @polymer
 */export const FrontendSopUser=superClass=>class extends superClass{fieldButtonClickedForSops(e){//console.log('optionPressed', e.detail.buttonName, 'selectedSampleAnalysis', e.target.value);                
...
datas.sopName=sopName;paramsUrl=paramsUrl+"&sopName="+sopName;break;default:return;}paramsUrl="&actionName="+actionName+"&finalToken="+this.finalToken+"&schemaPrefix="+schemaPrefix+paramsUrl;datas.schemaPrefix=schemaPrefix;datas.actionName=actionName;datas.paramsUrl=paramsUrl;//datas.callbackFunction=myCallbackFunctionTrigger.bind(this);
var tabInfo={currTabEsignRequired:this.currTabEsignRequired,currTabConfirmUserRequired:this.currTabConfirmUserRequired};datas.tabInfo=tabInfo;this.sopUserEndPoint(datas)}frontEndSopUserAPI(data){var apiUrl=backendUrl+frontEndSopUrl+"?"+data.paramsUrl;if(!data.finalToken){return}//    console.log('frontEndSopUserAPI', apiUrl, data.paramsUrl);
