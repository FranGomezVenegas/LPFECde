...
if(buttonDefinition&&buttonDefinition.confirmuser_required){store.dispatch(openConfirmUserDialog(this.actionTriggerNext.bind(this),this.actionTriggerAbort.bind(this)));return}this.actionTriggerNext()}actionTriggerAbort(){var message="";switch(this.selectedLanguage){case"es":message=this.validationNotCorrectMessage.message_es;break;//message=response.data.message_es; break;            
...
//this.sampleActionTrigger(actionName, datas, datas.tabInfo);                        
}this.$.prodLotBrowser.close();return}dialogClosedIncubatorAddTempReading(e){console.log("dialogClosedIncubatorAddTempReading triggered",e.detail,this.fieldsDialogSetSamplingDate);if("confirmed"==e.detail.dialogState){var actionName=e.detail.actionName,datas=[];// paramsUrl=paramsUrl+"&incubatorName="+selectedRow.selectedIncubator.name;    
...
//this.sampleActionTrigger(actionName, datas, datas.tabInfo);                        
}this.$.incubatorAddTempReading.close();return}}customElements.define("env-monit-elements",EnvMonitElements);