...
import{schema_name,em_browser_tabs,browserMain_programSelection,browserHome_defaultTab//    , sampleReception_sampleFieldToRetrieve, sampleReception_sampleFieldToDisplay
}from"../03config/config-process.js";import{FrontendEnvMonit}from"../01moduleFunctionality/frontend-env-monit.js";//import {setSelectedProgram} from '../02Redux/em-demo-a_actions.js';
import"./browser-tabs/em-demo-a-br-sample.js";import"./browser-tabs/em-demo-a-br-incubator.js";import"./browser-tabs/em-demo-a-br-batch.js";class emDemoABrowser extends FrontendEnvMonit(connect(store)(PolymerElement)){stateChanged(state){this.finalToken=state.app.user.finalToken;if(null!=state.tabs.tabs){this.appOpenTabs=state.tabs.tabs}if(null!=state.app.user.appProcedureList.procedures){this.procedure=state.app.user.appProcedureList.procedures[2]}this.selectedLanguage=state.app.user.appLanguage}static get properties(){return{schemaPrefix:{type:String,value:schema_name},thisTabName:{type:String,value:"em-demo-a-browser"},appOpenTabs:{type:String,observer:"onFinalTokenFilled"},finalToken:{type:String},programsList:{type:Array,notify:!0,bubble:!0,value:browserMain_programSelection},programs:Object,tabs:{type:Object,value:em_browser_tabs},currentTab:[],currentSubTab:{type:String,value:browserHome_defaultTab//currentTab: {type:Object, value:'home'},
