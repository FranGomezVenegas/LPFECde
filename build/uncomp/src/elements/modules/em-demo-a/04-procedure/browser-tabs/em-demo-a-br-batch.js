...
import{tableFieldLabel}from"../../03config/tablefield_labels.js";import{schema_name,browserBatchFieldToRetrieve,browserBatchFieldsToDisplay}from"../../03config/config-process.js";class emDemoABrBatch extends tableFieldLabel(FrontendEnvMonitSample(connect(store)(PolymerElement))){stateChanged(state){this.finalToken=state.app.user.finalToken;if(null!=state.emDemoA){if(null!=state.emDemoA.browserSelectedBatch){this.selBatch=state.emDemoA.browserSelectedBatch;//console.log('this.selBatch', this.selBatch);
