...
//}
}this.selectedLanguage=state.app.user.appLanguage;this.schemaPrefix=schema_name}static get properties(){return{callBackRefreshWindow:Object,finalToken:String,schemaPrefix:{type:String,observer:"onFinalTokenFilled"},selectedLanguage:String,allIncubators:{type:Array},incubationListElement:{type:Array,value:[{name:"shift",label_en:"Shift",label_es:"Turno",type:"list",dbType:"String",value:"Admin",read_only:!1,items:this.allIncubators}]},activeBatchesList:{type:Array,observer:"selectBatchWhenActiveBatchChanges"},activeBatchesFieldToDisplay:{type:Array,value:sampleIncubation_incubBatch_activeBatchFieldToDisplay},activeBatchesButtons:{type:Array,value:sampleIncubation_incubBatch_activeBatchButtons},selectedBatch:{type:Object},selectedBatchEmpty:{type:Object,value:selectedBatchEmpty},tableTitle:{type:Object,value:{label_en:"Active Batches",label_es:"Tandas Activas"}}}}static get template(){return html`
...
store.dispatch(setSelectedIncubator(data))}batchSelected(e){console.log("batchSelected",e);if(!e.detail.value){var noBatch=[];store.dispatch(setSelectedBatch(noBatch));return}//console.log('batchSelected', 'e.detail.value', e.detail.value);
this.selectedBatch=e.detail.value;store.dispatch(setSelectedBatch(e.detail.value));return}onFinalTokenFilled(){//this.callBackRefreshWindow = this.refreshWindow.bind(this);
