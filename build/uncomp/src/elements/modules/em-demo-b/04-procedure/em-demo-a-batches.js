...
var data=[];data.name="INC_1";store.dispatch(setSelectedIncubator(data))}batchSelected(e){if(!e.detail.value)return;//console.log('batchSelected', 'e.detail.value', e.detail.value);
this.selectedBatch=e.detail.value;store.dispatch(setSelectedBatch(e.detail.value));return}onFinalTokenFilled(){this.callBackRefreshWindow=this.refreshWindow.bind(this);//console.log('em-demo-a-batches >> calling getActiveBatches within onFinalTokenFilled');
